...
| **Update Project** | `PUT` | `/api/projects/:project_id` | **FORM-DATA:** `name` (text), `description` (text), `cover_image` (File) | Modifica la banda y su foto de portada. **Requiere ser Dueño o Admin.** |
...
| Petición | Método | URL | Body (JSON) | Descripción |
| :--- | :---: | :--- | :--- | :--- |
| **Create Post** | `POST` | `/api/posts` | **FORM-DATA:** `content` (text), `media` (File - Puedes agregar varias filas con 'media') | Crea una publicación nueva. Permite adjuntar hasta 10 imágenes al mismo tiempo. |
...
## 🔍 5. Buscador Global (`/api/search`)

| Petición | Método | URL | Body (JSON) | Descripción |
| :--- | :---: | :--- | :--- | :--- |
| **Search All** | `GET` | `/api/search?q=cyber&type=all` | *Ninguno* | Busca la palabra "cyber" tanto en Usuarios como en Bandas. |
| **Search Users** | `GET` | `/api/search?q=lake&type=users` | *Ninguno* | Busca la palabra "lake" solo en Usuarios. |
| **Search Projects** | `GET` | `/api/search?q=rock&type=projects` | *Ninguno* | Busca la palabra "rock" solo en Bandas (Nombre o Descripción). |
